...
class IntentRouter:
    ...
    def _load_registry(self):
        """โหลดทะเบียนทีมปฏิบัติการจากคลังความจำกลาง 00_memory"""
    ...
    def route_user_intent(self, user_message: str) -> str:
        ...
        ค้นหาจาก Keywords ใน Registry ของทีม หากไม่เจอจะใช้ Rule-based สแกนบริบทงาน
        """
        ...
        # 1. สแกนหาคำสำคัญ (Keywords Matching) ตามที่ตั้งไว้ในสารบัญความจำ
        ...
        # 2. Heuristic Fallback: ค้นหาคำกริยาเชิงวิจัยหรือการสร้างระบบ
        research_keywords = ["ช่วย", "หา", "วิจัย", "ทำ", "สร้าง", "ระบบ", "scout", "wellness"]
        if any(w in message_lower for w in research_keywords):
            print("⚠️ [Intent Router] ไม่เจอคำเจาะจง แต่บริบทเข้าข่ายงานวิจัยโครงสร้าง ส่งเข้า default: infrastructure_team")
